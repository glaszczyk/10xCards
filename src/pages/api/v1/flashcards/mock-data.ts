...
/**
 * Get mock flashcards with filtering and pagination
 */
export function getMockFlashcards(
  ...
  order: "asc" | "desc" = "desc"
): { data: FlashcardResponse[]; total: number } {
  // Filter by source if specified
  let filtered = source
    ? mockFlashcards.filter((card) => card.source === source)
    : mockFlashcards;

  // Sort the data
  filtered.sort((a, b) => {
    const aValue = sort === "createdAt" ? a.createdAt : a.updatedAt;
    const bValue = sort === "createdAt" ? b.createdAt : b.updatedAt;
...
// POST /flashcards mock functions
export function createMockManualFlashcard(data: {
  front: string;
  back: string;
  sourceTextId?: string;
}): FlashcardResponse {
...
export function createMockAIFlashcards(
  textContent: string
): FlashcardResponse[] {
  ...
  const mockCards = [
    ...
    {
      front: "What technology is mentioned in the text?",
      back: "The text mentions JavaScript, TypeScript, React, and other modern web technologies.",
    },
  ];

  return mockCards.map((card, index) => {
    const newId = `ai-mock-${Date.now()}-${index}`;
    const now = new Date().toISOString();
